...
class Tester {
public:
  Tester(LockFreeHashTable* table, int num_ops, int num_threads)
    : table_(table),
      num_ops_(num_ops),
...
TEST(Sequential, SimpleInsertion) {
  ...
  // on empty table
  uint32_t value;
  EXPECT_FALSE(l.lookup(7, value));
  EXPECT_TRUE(l.insert(7, 20));

  // on non-empty table
  EXPECT_TRUE(l.lookup(7, value));
  EXPECT_TRUE(value == 20);
  EXPECT_FALSE(l.lookup(8, value));
}
...
TEST(Sequential, SimpleDeletion) {
  ...
  uint32_t value = 0;
  // on empty table
  EXPECT_FALSE(l.remove(11));

  // on non-empty list
  EXPECT_TRUE(l.insert(72, 20));
  EXPECT_TRUE(l.insert(81, 40));
  EXPECT_TRUE(l.remove(72));

  EXPECT_FALSE(l.lookup(72, value));
  EXPECT_TRUE(l.lookup(81, value));
  EXPECT_TRUE(value == 40);

  // on newly empty list
  EXPECT_TRUE(l.remove(81));
  EXPECT_FALSE(l.lookup(45, value));
  EXPECT_FALSE(l.lookup(8, value));
}
...
TEST(Sequential, FailedDeletion) {
  ...
  EXPECT_TRUE(l.insert(17, 2));
  EXPECT_FALSE(l.remove(22));
}
...
TEST(Sequential, Reclaiming) {
  ...
  uint32_t reclaim_threshold = HazardPointers<int,2>::MAX_RETIRED_NODES_PER_THREAD;
  EXPECT_TRUE(l.insert(0,0));
  for (uint32_t i=1; i<2*reclaim_threshold; i++) {
    uint32_t value = 0;
    EXPECT_TRUE(l.lookup(i-1, value));
    EXPECT_TRUE(value == (i-1));
    EXPECT_TRUE(l.insert(i,i));
